...
<script>
    function orderGoods(id){
        $.ajax({
            ...
            success: function(data){
               var  tr = "<input type='hidden' id='choose-order' value='"+id+"'><table cellpadding='3' cellspacing='1'><tr><th><input name='split' type='checkbox' value='1' onclick='judgeSplit(this)' />拆分发货</th><th>商品图片</th><th>商品名称</th><th>商品规格</th><th>购买数量</th></tr>";
               $(data).each(function (k,v) {
                   tr += "<tr class='tron'>";
                   tr += "<td align='center'><input name='id' type='checkbox' value='"+v.id+"' class='goods_list' style='display: none' /></td>";
    ...
    function comDeli() {
        ...
        $('input[name="id"]:checked').each(function(){
          ...
          split = 0;
        $('input[name="split"]:checked').each(function(){
           split = $(this).val();
        });
            ...
            dataType: 'json',
            data: {express:express,orderId:orderId,split:split,idstr:idstr},
            success: function(data) {
                window.parent.frames.main-frame.location.reload();
                if (data == 'success'){
                    $('#goods-list').css('display','none');
                    alert('发货成功!');
                }else if(data == 'noGoodsId'){
                    alert('请选择拆分发货商品!');
                }else{
                    alert('发货失败！');
                }
    ...
    function judgeSplit(obj) {
        if($(obj).is(':checked')){
            $('.goods_list').css('display','inline');
        }else{
